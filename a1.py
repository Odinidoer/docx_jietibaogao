			...
			for j in range(len(docx.tables[i].rows)):
				...
				if u'实验室导师' in docx.tables[i].rows[j].cells[0].text:
					if u'电话' in docx.tables[i].rows[j].cells[2].text :
						run_teacher = docx.tables[i].rows[j].cells[1].paragraphs[0].runs[0]
						run_teacher.text = tutor_info[0]
						run_teacher.font.size = font_size
						run_teacher.font.name = font_name
						run_phone = docx.tables[i].rows[j].cells[3].paragraphs[0].runs[0]
						run_phone.text = tutor_info[1]
						run_phone.font.size = font_size
						run_phone.font.name = font_name
						run_email = docx.tables[i].rows[j+1].cells[3].paragraphs[0].runs[0]
				...
				if u'项目联系人' in docx.tables[i].rows[j].cells[0].text:	
					if u'电话' in docx.tables[i].rows[j].cells[2].text :	
						run_contactor = docx.tables[i].rows[j].cells[1].paragraphs[0].runs[0]
				...
				if u'销售员' in docx.tables[i].rows[j].cells[0].text:
					if u'电话' in docx.tables[i].rows[j].cells[2].text :
						run_seller = docx.tables[i].rows[j].cells[1].paragraphs[0].runs[0]
				...
				if u'技术支持' in docx.tables[i].rows[j].cells[0].text:
					if u'电话' in  docx.tables[i].rows[j].cells[2].text :
						run_supporter = docx.tables[i].rows[j].cells[1].paragraphs[0].runs[0]
...
def _MJ_tables(docx):
	for i in range(len(docx.tables)):
		...
		#4.1.1
		##GO.list
		__MJ_table(i,docx,u'蛋白Accession号',u'对应的GO编号','%s/2.Annotation/2.1.GO/GO.list' %(args.file),r'GO:' ,5,table_format)
		##*level2/3/4.xls
		__MJ_table(i,docx,u'GO注释分类的分支',u'GO分类的定义','%s/2.Annotation/2.1.GO/GO.list.level2.xls'%(args.file),r'GO:',10,table_format)
		#4.1.2
		##pathway.txt
		__MJ_table(i,docx,u'蛋白的Accession编号',u'对应的KO号','%s/2.Annotation/2.2.KEGG/pathway.txt'%(args.file),r'K',10,table_format)
		##pathway_table.xls
		__MJ_table(i,docx,u'通路的编号',u'通路的定义','%s/2.Annotation/2.2.KEGG/pathways/pathway_table.xls'%(args.file),r'K',5,table_format)
		##kegg_table.xls
		...
		__MJ_table(i,docx,u'Id',u'Enrichment','%s/3.DiffExpAnalysis/3.2.GO/Enrichment/*_vs_*.diff.exp.xls.DE.list.enrichment.detail.xls'%(args.file),r';',4,table_format)
		#4.2.3
		##*.pathway.xls
...
def _MJ_jpgs(docx):
	...
	for i in range(len(docx.paragraphs)):
		...
		##GO二级、三级、四级分类统计九饼图
		__MJ_jpg(i,docx,u'level234.pdf：GO二级、三级、四级分类统计九饼图','%s/2.Annotation/2.1.GO/GO.list.Level234.pdf' %(args.file))
		#4.1.2
		##包含蛋白数目最多的前20个通路
		__MJ_jpg(i,docx,u'pathway.top20.pdf：包含蛋白数目最多的前20个通路（除以ko01开头的基础通路外）','%s/2.Annotation/2.2.KEGG/pathway.top20.pdf'%(args.file))
		##KEGG通路图片展示
		__MJ_jpg(i,docx,u'pathways文件夹下的.png文件：KEGG通路图片展示','%s/2.Annotation/2.2.KEGG/pathways/*.png'%(args.file))
		##KEGG代谢通路进行分类
		__MJ_jpg(i,docx,u'kegg_classification.pdf：对蛋白做KO注释后，可根据它们参与的KEGG代谢通路进行分类','%s/2.Annotation/2.2.KEGG/kegg_classification.pdf'%(args.file))
		#4.1.3
		##COG功能分类统计柱图
		__MJ_jpg(i,docx,u'COG.class.catalog.pdf：COG功能分类统计柱图','%s/2.Annotation/2.3.COG/COG.class.catalog.pdf'%(args.file))
		##KOG功能分类统计图
		__MJ_jpg(i,docx,u'KOG.class.catalog.pdf：KOG功能分类统计柱图','%s/2.Annotation/2.3.KOG/KOG.class.catalog.pdf'%(args.file))
		#4.2.1
		##差异蛋白可视化火山图
		...
		#4.2.3
		##差异蛋白KEGG通路图片展示
		__MJ_jpg(i,docx,u'*.png：差异蛋白KEGG通路图片展示','%s/3.DiffExpAnalysis/3.3.KEGG/Annotation/*_vs_*.diff.exp.xls.path/*.png'%(args.file))
		##分组差异蛋白KEGG pathway富集分析柱状图
		__MJ_jpg(i,docx,u'*.pathway.pdf：各分组差异蛋白KEGG pathway富集分析柱状图','%s/3.DiffExpAnalysis/3.3.KEGG/Enrichment/*pathway.pdf'%(args.file))
		#4.2.4
		##差异蛋白表达模式聚类图
		__MJ_jpg(i,docx,u'Heatmap.pdf：差异蛋白表达模式聚类图','%s/3.DiffExpAnalysis/3.4.Cluster/Heatmap.pdf'%(args.file))
		##差异蛋白模块（clusters）表达趋势折线图
		__MJ_jpg(i,docx,u'Heatmap_trendlines_for_*_subclusters.pdf：差异蛋白模块（clusters）表达趋势折线图','%s/3.DiffExpAnalysis/3.4.Cluster/*subclusters.pdf'%(args.file))
		#4.2.5
